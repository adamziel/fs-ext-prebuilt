...
//   PREBUILDIFY_ARCHES  - comma/space list of architectures (default: current arch)
...
const ABI_REGISTRY_URL = process.env.ABI_REGISTRY_URL || 'https://raw.githubusercontent.com/nodejs/node/main/doc/abi_version_registry.json';
const ABI_REGISTRY_PATH = process.env.ABI_REGISTRY_PATH || path.join(ROOT, 'abi-registry.json');
const STUDIO_PACKAGE_URL = process.env.STUDIO_PACKAGE_URL || 'https://raw.githubusercontent.com/Automattic/studio/trunk/package.json';
const NODE_VERSIONS_ENV = process.env.NODE_VERSIONS || '';
const ELECTRON_VERSION_ENV = process.env.ELECTRON_VERSION || '';
const ARCH_LIST = (process.env.PREBUILDIFY_ARCHES || process.arch)
    .split(/[,+\s]+/)
    .map((arch) => arch.trim())
    .filter(Boolean);
        ...
        https
            .get(url, (res) => {
                ...
                if (statusCode < 200 || statusCode >= 300) {
                    res.resume();
                    return reject(new Error(`Request failed for ${url} with status ${statusCode}`));
                }
...
async function ensureAbiRegistry() {
    const shouldRefresh = process.env.REFRESH_ABI === '1' || !fs.existsSync(ABI_REGISTRY_PATH);
...
async function resolveElectronVersion() {
    ...
    const electron =
        studioPackage?.devDependencies?.electron || studioPackage?.dependencies?.electron || '';
...
function runPrebuild(targetLabel, targetSpec) {
    ...
    const args = ['npx', '--yes', 'prebuildify'];
    ...
    if (!osDirs.length) {
        throw new Error('No prebuild outputs found. Expected prebuilds/<platform>-<arch>');
    }
    ...
    osDirs.forEach((dirent) => {
        ...
        const artifacts = fs.readdirSync(buildDir).filter((file) => file.endsWith('.node'));
...
(async () => {
    ...
    if (!nodeVersions.length) {
        throw new Error('No Node.js versions selected. Set NODE_VERSIONS to override.');
    }

    console.log(`Building prebuilds for Node.js versions: ${nodeVersions.join(', ')}`);
    console.log(`Electron target: ${electronVersion}`);
    console.log(`Architectures: ${ARCH_LIST.join(', ')}`);
